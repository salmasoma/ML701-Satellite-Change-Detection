...
ST_COLORMAP_TUPLES = [tuple(color) for color in ST_COLORMAP]
ST_CLASSES = ['No Change', 'water',   'N.v.g. surface',  'low vegetation', 'tree',      'building', 'Playground']
...
def Eval():
    ...
    INFER_DIR1 = './second_augmented/pred_aug/im1_rgb/'  # Inference path1
    INFER_DIR2 = './second_augmented/pred_aug/im2_rgb/'  # Inference path2
    LABEL_DIR1 = './second_augmented/test/label1/'  # GroundTruth path
    LABEL_DIR2 = './second_augmented/test/label2/'  # GroundTruth path
    ...
    for infer, gt in tqdm(zip(infer_list, label_list)):
        ...
        structured_array = np.core.records.fromarrays(infer_array.transpose(), 
                                              names='r, g, b', 
                                              formats='i8, i8, i8')

        # Find unique rows (triplets)
        unique_triplets = np.unique(structured_array)
        standard_triplets = [tuple(row) for row in unique_triplets]
        #get index of each element in ST_COLORMAP
        unique_set = {ST_COLORMAP_TUPLES.index(color) for color in standard_triplets if color in ST_COLORMAP_TUPLES}
    ...
    print('Accuracy = ', acc_meter.avg*100)
