...
class SSCDe(nn.Module):
    def __init__(self, in_channels=3, num_classes=7):
        super(SSCDe, self).__init__()
    ...
    def forward(self, x1, x2):
        ...
        change_upsampled = F.interpolate(change, size=x_size[2:], mode='bilinear', align_corners=True)
        change_upsampled_squeezed = torch.squeeze(change_upsampled, 1)  # Remove channel dimension if necessary
        ...
        upsampled_out1 = F.interpolate(out1, size=x_size[2:], mode='bilinear', align_corners=True)
        upsampled_out2 = F.interpolate(out2, size=x_size[2:], mode='bilinear', align_corners=True)

        return change_upsampled_squeezed, upsampled_out1, upsampled_out2
