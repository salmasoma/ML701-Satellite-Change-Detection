...
#from models.HRSCD4 import HRSCD4 as Net
#from models.HRNet import HRNet as Net
DATA_NAME = 'Second BiSRNet'
#################################
...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--test_dir', required=False, default='./second_augmented/test', help='directory to test images')
        parser.add_argument('--pred_dir', required=False, default='./second_augmented/pred_aug', help='directory to output masks')
        parser.add_argument('--chkpt_path', required=False, default=working_path+'/checkpoints/Second Augmented BiSRNet/BiSRNet_29e_mIoU70.04_Sek16.39_Fscd56.87_OA87.11.pth')
...
def predict(net, pred_set, pred_loader, pred_dir, flip=False, index_map=False, intermediate=False):
    ...
    for vi, data in enumerate(pred_loader):
        ...
        imgs_A = imgs_A.to(device, dtype=torch.float32)
        imgs_B = imgs_B.to(device, dtype=torch.float32)
